...
if [ -n "$2" ]; then                # cut if asked for
    eval ffmpeg -y -hide_banner -loglevel warning $timeargs -i temp00001 tempvideo_cut.mp4
    eval ffmpeg -y -hide_banner -loglevel warning $timeargs -i temp00002 tempaudio_cut.m4a
    eval ffmpeg -y -hide_banner -loglevel warning -i tempvideo_cut.mp4 -i tempaudio_cut.m4a combined.mp4
else
    eval ffmpeg -y -hide_banner -loglevel warning -i temp00001 -i temp00002 combined.mp4
fi
rm temp*
