...
parser.add_argument("-f", "--filter", default="", help="print emoji only if name contains filter")
parser.add_argument(
    "-l", "--limit", default=0, type=int, help="limit the number of output emojis, defaults to 0 (no limit"
)
parser.add_argument("-N", "--no-name", default=False, action="store_true", help="do not output emoji names")
parser.add_argument(
    "-w",
    "--wipe",
    default=False,
...
parser.add_argument("-c", "--lower-case", default=False, action="store_true", help="convert output to lower case only")
parser.add_argument("-i", "--input", help="path to file with additional newline separated content")
...
output = ifile.read_text() if args.input and ifile.exists() else ""
